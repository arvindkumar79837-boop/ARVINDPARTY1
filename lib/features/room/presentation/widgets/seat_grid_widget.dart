...
class _SeatTile extends StatelessWidget {
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: speaking ? const Color(0xFFFF8906) : Colors.transparent,
                      width: 2.5,
                    ),
                    boxShadow: speaking
                        ? [
                  ...
                  child: CircleAvatar(
                    ...
                    backgroundImage: occupied && seat.avatar != null && seat.avatar!.isNotEmpty
                        ? NetworkImage(seat.avatar!)
                        : null,
                    child: !occupied
                        ? Icon(
                            seat.isLocked ? Icons.lock_outline : Icons.mic_none_outlined,
                            color: seat.isLocked
                ...
                if (occupied && seat.userId != null)
                  Positioned(
                    top: 0,
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1),
                      decoration: BoxDecoration(
                        color: seat.isHost ? const Color(0xFFFF8906) : Colors.cyanAccent,
                        borderRadius: BorderRadius.circular(6),
                      ),
                      child: Text(
                         seat.userId == 'host' ? 'HOST' : 'SEAT',
          ...
          // ── Name / Seat number ───────────────────────────────────────────
          Text(
            occupied ? (seat.userName ?? '') : '${seat.index + 1}',
            style: TextStyle(
  ...
  void _onTap(RoomController ctrl) {
    if (!seat.isOccupied) {
      ...
      // Logic handling to match dynamic active member runtime session
      ...
      _BottomSheet(
        ...
        children: [
          _SheetTile(
            ...
            onTap: () {
              Get.back();
              // FIXED: Removed extra dynamic mock parameters to align cleanly with controller's contract
              ctrl.takeSeat(index); 
            },
          ),
